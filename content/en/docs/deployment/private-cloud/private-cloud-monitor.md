...
This document will help you quickly set up a solution for monitoring Mendix on Kubernetes environments.
...
| Mode                      | Native           | Compatibility (deprecated) |
| ------------------------- | ---------------- | -------------------------- |
| Mendix Operator version   | v2.4.0 and above | v2.1.0 to v2.26.1          |
...
Setting the metrics generation of a Mendix on Kubernetes environment to `native` mode will collect Prometheus metrics directly from the Mendix Runtime.
...
When metrics generation is set to `compatibility` mode, Mendix on Kubernetes adds an additional `m2ee-metrics` sidecar to the environment's pods.
...
For more details see the [Loki installation guide](https://grafana.com/docs/loki/latest/setup/install/).
...
You can use the same wildcard domain name as other Mendix apps - for example, if you're using *mendix.example.com* as the Mendix on Kubernetes domain name,
...
* (deprecated) [compatibility mode dashboard](https://cdn.mendix.com/mendix-for-private-cloud/grafana-dashboard/mendix_app_dashboard_compatibility-1.1.1.json) for metrics generated in compatibility mode
...
{{% alert color="info" %}}Starting from Mendix Operator v2.27.0, only the `native` mode is supported.
Installing or upgrading to Operator v2.27.0 or a later version will switch the default metrics mode to `native`.{{% /alert %}}

{{% alert color="info" %}}This way, upgrading an older Mendix Operator will not change the way it generates metrics.{{% /alert %}}
...
7. Set **Mode** to **native**, then click **Save**.
