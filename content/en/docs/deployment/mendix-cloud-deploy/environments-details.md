...
* **Plan** – the type of plan covered by your license (for more information, see the [Plan Details](#plan-details) section below)
...
In addition to the above information, there are three more grids showing information about the [deployment package details](#deployment-package-details), [plan details](#plan-details), and [license](#license).
...
* **View Live Log** – This shows a live log for your application. It performs the same action as the **View Live Log** button on the [Logs](/developerportal/operate/logs/) page.
...
* **View Live Log** – This shows a live log for your application.
...
To see the options for changing scaling, scroll down to the **Instances** row of the **General** tab and click **Change scaling**.

{{< figure src="/attachments/deployment/mendix-cloud-deploy/environments-details/change-scaling.png" alt="" class="no-border" >}}

In the **Change scaling** dialog box that appears next, there are two sliders that you can control:
...
Constants are used to define configuration values that can differ per environment. In this section, you can view the configured constants and export the constant values to an Excel file by clicking **Export To Excel**.
