...
* (General) We have fixed an issue where licenses with an expiration date past 2038 are not processed correctly after upload.
* (General) We have fixed an issue with the debugger URL.
* (General) We have fixed an issue with the environment URL.
* (General) We have fixed an issue where users could see App Metrics and Logs even when their access groups did not allow it.
