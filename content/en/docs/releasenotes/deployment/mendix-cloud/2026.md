...
* We have introduced updates to support centralized certificate management in future release. Going forward, users will be able to manage custom domain certificates centrally, rather than only at the application level. To enable this, the Mendix Portal has implemented UI changes on the **Cloud Settings → Custom Domains page**. The updated UI now clearly indicates whether a certificate is managed at app level (locally) or centrally.
