...
- We fixed an issue where an app crash occurred when a **Call a microflow** event was selected then the select button was clicked in the Map entity dialog. (Ticket 225433)
...
- We fixed an issue where the page editor's **Generate validation microflow** feature would show an Oops dialog.
...
- We fixed an issue where an app using the React client would not get updated correctly in production due to a caching issue.
