...
* We added [mx.exe git-merge command](/refguide10/mx-command-line-tool/merge/#git-merge) to use as a custom git merge driver for MPRv2. For now it supports merging MPRv2 with MPRv2 and MPRv1 with MPRv1. 
* We now support microflows as a parameter type for JavaScript actions. Microflow parameters work the same as the already existing Nanoflow parameters. When a microflow is called from a JavaScript action a [runtime operation](/refguide10/communication-patterns/#RO) is used.
