...
- <a id="fix-debugger-reconnect"></a>We fixed a [known issue](#ki-debugger-reconnect) where the debugger did not reconnect when restarting an app using the React client.
...
- We fixed an issue where the debugger did not reconnect when restarting an app using the React client.
- We fixed an issue when locally rerunning with runtime-only changes while using the React client. They are now properly loaded by the runtime.
...
* In the React client, the debugger does not reconnect when restarting an app.
    * Fixed in [10.24.4](#fix-debugger-reconnect).
    * Fixed in [10.24.4](#fix-debugger-reconnect).
    * Fixed in [10.24.4](#fix-debugger-reconnect).
    * Fixed in [10.24.4](#fix-debugger-reconnect).
    * Fixed in [10.24.4](#fix-debugger-reconnect).
