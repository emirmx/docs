...
{{% alert color="info" %}}For an overview of the highlights in this release, see the [Mendix Release 10.24](https://www.mendix.com/blog/mendix-release-10-24-studio-pro-a-stable-ide-for-modern-enterprise-development/) blog post.{{% /alert %}}
...
{{% button color="info" href="https://marketplace.mendix.com/link/studiopro/10.24.19" text="Go to Marketplace" title="Download version 10.24.19 from the Marketplace" %}}
...
* We improved the performance of opening the entity selector dialog box. (Ticket 270854)
...
* We introduced an opt-in runtime setting `MaxRetrieveAmount` that limits the maximum amount of objects retrieved by database data source operation. (Tickets 236618, 247111)   
...
* Studio Pro has been updated to run on .NET 10 instead of .NET 8. As a result, .NET 10 is now a required [prerequisite](/refguide10/system-requirements/) for the application.
...
* In the logic editors, we fixed an issue affecting users [extracting submicroflows](/refguide10/extracting-and-using-sub-microflows/) if the sequence flow in the original microflow had invalid condition value, and the extracted microflow was missing one. (Ticket 269233)
...
* The Chinese, Korean, and Japanese versions of the Studio Pro UI are now in General Availability. Please see [documentation](/refguide10/preferences-dialog/#language) for more information.
