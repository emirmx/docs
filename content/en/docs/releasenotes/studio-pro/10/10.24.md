...
* We fixed the issue that the runtime log levels were not restored correctly when restarting the app from Studio Pro, in case the runtime was stopped earlier due to the trial version timeout. (Ticket 251399)
...
* There is a known issue with the React client where locally re-running an app with runtime-only changes does not properly apply those changes. (Tickets 253539, 253932, 253176, 253842, 254100)
* There is a known issue with the React client where the debugger would not reconnect when restarting an app.
