...
- We fixed the issue where `dbms_lob.substr` was unnecessarily applied to limited String attributes when running SQL queries in Oracle
...
- When upgrading to a data widgets module version 3.0.0 or above, we fixed an issue where the configuration of drop-down filters settings for associations on data grid 2 columns was not converted correctly to the drop down-filter widget of the new setup during data grid 2 widget updates.
