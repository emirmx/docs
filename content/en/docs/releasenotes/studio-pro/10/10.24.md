...
* There is a known issue with the React client where locally re-running an app with runtime-only changes does not properly apply those changes.
* There is a known issue with the React client where the debugger disconnects when restarting an app.
