...
- We improved the authentication mechanism in STP. It fixes the following support issues: 
...
- We fixed an issue where an entity with **Synchronization Config** set to **Never** would be removed from the list view or data grid 2 when **Synchronize All** is triggered (due to excessive cache deletion). For more information on synchronization, see [Offline Synchronization](/refguide/mobile/building-efficient-mobile-apps/offlinefirst-data/synchronization/). (Ticket 253629)
...
- We fixed an issue where an enumeration with a value or caption set to `com` would result in a compilation error when you try to run the app. We have also added this to the [reserved words](/refguide10/enumerations/#name) list. (Ticket 255976)
