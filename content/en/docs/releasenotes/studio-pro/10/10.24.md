...
- We improved the authentication mechanism in STP. It will fix the following support issues: 
...
- We improved performance when summarizing conflict information in huge documents.
...
- We fixed an issue where an entity with **Synchronization Config** set to **Never** would be removed from the list view or data grid 2 when Synchronize All is triggered, due to excessive cache deletion. (Ticket 253629)
...
- We fixed an issue in external OData entity's object initialization where specifying the values for keys via microflow expression would fail.
