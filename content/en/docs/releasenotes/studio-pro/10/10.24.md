...
* We fixed an issue on offline associated entities cause startsWith is not a function error in synchronize actions (Ticket #257346)
...
* We fixed an issue where consistency error for offline profile where "Specialization Entity should have an offline synchronization mode as its generalization Entity has an offline synchronization mode" was wrongly reported. (Ticket #255323)
* We fixed the issue where importing modules with some files that have long paths and did not compose within path limits failed to throw an error message. It will fix below mentioned support issues.
