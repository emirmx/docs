...
- We fixed a conversion when upgrading apps from 9.24 for arguments of **Show a page**, **Call a microflow**, or **Call a nanoflow** [event actions](/refguide/on-click-event/#actions) where parameter mappings changed to a different selection variable on the same page. (Ticket 253494) {TW}
...
- In Dojo client, we fixed pop-up window resizing when interacting with a Combobox inside the popup on mobile devices. {TW}
- We fixed a conversion issue with sorting in the **Gallery** widget when upgrading the data widgets Marketplace module to version 3.0.0 or above. {TW}
