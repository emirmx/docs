...
- In the logic editors, we improved the scrollbar styling as well as interaction when clicking on the scrollbar track.
- In the logic editors, we fixed some layouting glitches when adding objects to flows.
- In the logic editors, we now move connected elements out of the way when an element is resized.
