...
- We fixed an issue where some task queue tasks remain in `Running` state when a clustered application is restarted while optimistic locking is also enabled (Ticket 239838).
...
- Improved a11y in popup modal by allowing keyboard to reach the close button.
