...
- We added a new menu option **Help** > **Support Tools** > **Refetch Repository** to fix the corrupted repository due to LibGit2Sharp.NotFoundException exception. {TW}
...
- We fixed an issue in the workflow engine where it was impossible to continue a workflow that became incompatible while it was failed, when event handlers were not set. {TW}
...
- We fixed an issue where you could configure the [default tab attribute](/refguide/tab-container/#attribute) with a page parameter resulting in a deployment error. The selector of this attribute will no longer allow selecting a page variable directly, but the attribute must be accessed via a data view. {TW}
- We fixed an issue where the [default tab attribute](/refguide/tab-container/#attribute) was not being properly validated and could lead to incorrect behavior during deployment or at runtime. These scenarios are now correctly checked and result in a consistency error. {TW}
...
- We fixed an issue in the runtime where published OData and REST services accepted the credentials of web service users. The intended purpose of web service users is for them to be able to access published web services only. Role-based access was still be applied, but for web service users that had roles with access to OData and/or REST services, this allowed them to access these services. Apps that currently have web service users accessing published OData and/or REST services should delete these web service users and recreate them as regular users. In cases where this is not feasible, we have introduced a runtime setting `com.mendix.services.publish.AllowWebServiceUserBasicAuthenticationInODataAndREST` (default: *false*). Setting it to *true* reverts to the existing behavior. 
    This setting is relevant only for apps that:
        1. Were first built in Studio Pro 10.2.0 or later, because this issue was introduced in Studio Pro 10.2.0,
            ...
            4. Are unable to delete these web service users and recreate them as regular users. {TW}
...
-  We fixed the issue for pull operation when PMP configuration setup. {TW}
...
- We fixed an issue where certain JavaScript actions that were meant for both web and native platforms faiedl to bundle when using the React client.
...
- We removed the space from the caption of the buttons column while generating the contents of the Data Grid 2. {TW}
