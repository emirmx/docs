...
- We added Cpu, Gpu, memory, and additional information to new gather support information tool. It is also now possible to save a copy of the support information for ease of use so users can simply send the support info directly to support if needed.
...
- In the Security overview, we now show the pages, microflows, and nanoflows accessible to a selected user role in a selected module.
- We added a module roles tab to the [Security Overview (Beta)](/refguide/security-overview/). This tab provides a overview of all module roles assigned to a user role within the selected module.
- We added an Excel export to the [Security Overview (Beta)](/refguide/security-overview/). This export contains all access rules, the access to pages, nanoflows and microflows, and the module role mapping.

### Improvements

- We added the `sync-java-dependencies` command to the `mx` command-line tool. Using this command you can synchronize managed Java dependencies.
- We’ve improved deployment speed for non-progressive web applications.
...
- In the integration pane in Studio Pro, OData services are now indicated with the subtitle "OData", and business event services are indicated with the subtitle "AsyncAPI".
...
- In the integration pane in Studio Pro, associated entities that are used in the app now show a green checkmark.
- In Studio Pro we moved the menu item for OpenAPI Documentation in the View menu into its correct alphabetical position.
- In the logic editors, we made the Logic Recommender less intrusive: The blue plus is hidden when the flow is too short to allow the flow to be selected. It is hidden when the flow is selected to allow the bezier handles to be adjusted. To show the Logic Recommender in this case, we added a context menu option.
- In the logic editors, when a flow, empty looped activity or "new" flow is selected, the Logic Recommender will now always open with Ctrl-Enter.
...
- Maia chat references are now collapsed by default. Also, Maia chat max-width was removed and reference toggle was updated according to the new design.
...
- We've reduced the number of service calls required to fetch necessary information, enhancing and optimizing performance during VCS operations.
...
- In the integration pane in Studio Pro, searching the Catalog and clicking 'Show more' will now show actions, and allow you to drag them to a microflow.
