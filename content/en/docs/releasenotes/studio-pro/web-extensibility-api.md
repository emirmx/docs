...
- We introduced a change for the Model API that makes it easier to create Import Mappings and Export Mappings based on JSON Structure documents. This includes `addJsonStructure` that allows you to create a JSON Structure from JSON content and `getElements` to see the elements inside a JSON Structure document. It also includes `addImportMapping` and `addExportMapping` to create mapping documents from a JSON Structure, and `setElementMapping` and `clearElementMapping` to later update the mapped elements for mapping documents.
...
- We introduced a new Progress Dialog API under `studioPro.ui.dialogs.showProgressDialog(<title>, <steps>)`, which opens a modal containing a series of steps that perform actions in sequence. For more information, see the [Showing a Progress Dialog](/apidocs-mxsdk/apidocs/web-extensibility-api-11/dialog-api/#process-dialog) section of _Open a Modal Dialog Using Web API_.
