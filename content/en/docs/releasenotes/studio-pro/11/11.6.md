...
- We fixed an issue when an older offline client (from a Mendix version before 11.6.5) would fail to synchronize with a HTTP 560 (Invalid JSONRequest) error, preventing the offline app from loading. (Ticket 274811)
...
- We fixed an issue with the client where passing an empty string as a parameter to a microflow triggered an error. For example, when filtering options of a combo box with a microflow, clearing the search string would trigger the error. This would occur for apps with the [new string behavior](https://docs.mendix.com/refguide/runtime-tab/#new-string-behavior) disabled. (Ticket 271819)
