...
This happens when the deployment package (MDA) has been created using [MxBuild](/refguide/mxbuild/) which is also the tool used to build apps in the Mendix Portal and through [Mendix Pipelines](/developerportal/deploy/mendix-pipelines/). The same app can be run locally without this issue.

Mendix is working to find a solution.
