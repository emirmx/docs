...
#### Cloud Deployment on Upgrade

##### 'key not found: EnableNewStringBehavior' Error.

When an app is upgraded to 11.6 and deployed to the cloud, the deployment can fail with `ERROR - Core: Failed creating an instance of 'com.mendix.webui.model.settings.ModelWebUIProjectSettingsPart'.
java.util.NoSuchElementException: key not found: EnableNewStringBehavior`

This happens when the deployment package (MDA) has been created using [MxBuid](/refguide/mxbuild/) which is also the tool used to build apps in the Mendix Portal and through [Mendix Pipelines](/developerportal/deploy/mendix-pipelines/). The same app can be run locally without this issue.

##### Cloud Deployment Workaround

A workaround is to deploy your app through Studio Pro, using the [Deploy to Licensed Cloud Node](/refguide/app-menu/#deploy) option.
