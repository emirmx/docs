...
- We introduced a new UI in integration pane to separate catalog services from already consumed services. The new UI shows them in dedicated segments with independent search functionality.
...
- When manually resizing a popup dialog in Studio Pro, it will remember the new size and use it next time the dialog is opened again. This feature can be switched off with a new setting in the 'Environment environment' tab of the 'Preferences' dialog. To forget the stored dialog sizes again, an option is added to the 'Restore Default Layout' dialog that can be opened from the 'View' menu.
...
- We switched from the IndexedDB storage for the Offline PWA database to OPFS storage. It will enhance the performance of the synchronization and SQL queries in general.

  NOTE: The first time when the Offline PWA is opened/refreshed on this new Mendix version, will trigger the migration of data from the IndexedDB storage. It will take some time and the application start will be delayed until that is finished, depending on the size of the offline database for that browser.
- We added support for the following [languages](/refguide/language-settings/): Georgian.
...
- We improved the way Studio Pro updates attributes and associations based on the OQL query of a view entity. It now renames existing attributes and assocations when the expression is the same but the alias is different.
...
- Mendix 11 introduces a change in the versioning schema by dropping the fourth element, known as the build number.
  Versions will now follow a SemVer pattern `major.minor.patch` with the occasional pre-release suffix e.g. `beta.1` for
  planned pre releases. This semantic version will also be the version that is returned by Core.getRuntimeVersion(). Apps build with
  a pre-release can be identified internally by their exact version.
...
- We removed unnecessary auto scroll after converting/creating/duplicating a document if document is already in the view in the modernized app explorer.
...
- We fixed an issue where the search criteria weren't properly cleared when closing the beta history pane.
