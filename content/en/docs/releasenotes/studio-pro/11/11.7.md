...
* We improved the performance of document operations (add, rename, delete, etc.) in App Explorer.
* We added support for inline and block commenting in OQL for improved query readability. Use `--` for single-line comments and `/* … */` for multi-line comments with nesting.

    The Editor shortcuts are:
...
* We fixed the issue in modernized app explorer where pressing <kbd>ENTER</kbd> on a leaf node didn't open a document.
