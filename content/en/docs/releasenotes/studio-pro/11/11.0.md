...
- You can use Maia to help you generate workflows by using text/image prompts.
...
- We fixed an issue where an edited value was not saved in a dialog when the cell lost focus (for example, when you clicked a cell to edit value, edited it, and then clicked an arrow button to expand a value).
...
- We fixed an error pop-up that appeared after clicking **Cancel** in a progress dialog.
...
- In the logic editors, we moved connected elements out of the way when an element is resized.
