...
- We made the context menu that opens in the page editor or microflow editor to support navigation using the arrow keys.
...
- We changed the internal representation in the client of an *empty value* from an empty string to `null`. This fixes an issue where empty string attributes became an empty string value when an object was communicated to the client. Due to this change, both `MxObject#get` and `MxObject#set` have changed to accept/return `null` for empty attributes.
...
* We implemented the **Maia for Workflows** where users can generate workflows by text and image prompts.
