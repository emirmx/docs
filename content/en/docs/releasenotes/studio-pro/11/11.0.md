...
- The React client is now set as the default client when creating a new application from Studio Pro.
We have released the interrupting boundary events feature for Workflows. With this new type of boundary event the workflow can interrupt modeled flows when the timer has expired and start a alternate flow.
...
We implemented the Maia for Workflows agent where users can generate workflows by text/image prompts.
...
- We added a selector to the edit view entity dialog that allows you to specify a user for whom you want to see the live preview. This allows you to preview queries that use the [%CurrentUser%] token.
...
- We have updated the version of Mendix apps that can be opened with Mendix 11.0.0 and later. Now, to open (and if necessary convert) an app in Mendix 11.0.0 and later, it should be of version 10.21.0 or higher.
...
* We upgraded the JDBC driver for Microsoft SQL Server to version 12.10.0.
...
- We have enabled entity access on the ShowHomePage microflow in the system module (that is automatically included in the app). This reduces security concerns by providing a secure default. (Ticket 92564)
