...
- We've fixed an issue that the offline client was not able to detect a change in the domain model causing SQL request failures. (Ticket 220074)
...
- We fixed an error that could appear when expanding nodes in the tree control.
...
- We've fixed a problem where the user opened the number input in the date picker in voiceover mode, it wasn't possible to close it.
