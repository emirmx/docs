...
* **`src/ui/index.tsx`** — The React component rendered inside the Studio Pro tab. In the template it renders a simple heading and paragraph — replace it with your own UI. If you rename this file, update two places: the entry point in `build-extension.mjs`, and the `uiEntrypoint` value passed to `studioPro.ui.tabs.open(...)` in `src/main/index.ts`. You can delete this file entirely if you also remove both of those references.
