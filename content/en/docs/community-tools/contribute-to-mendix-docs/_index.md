...
Before starting with this how-to, make sure you have a GitHub account. If you do not have one yet, sign up at [Join GitHub](https://github.com/join).
...
The [Editing the Documentation and Previewing Changes Locally](#local-build) section covers how to fork and clone the [mendix/docs](https://github.com/mendix/docs) repository, edit documentation locally, and build local previews.
...
1. Click **View on GitHub** ({{< icon name="notes-paper-edit" >}}) in the upper right corner of the specific Mendix documentation page that you want to work on.
    ...
    The corresponding file opens on GitHub.
...
2. Click **Edit this file** ({{< icon name="pencil" >}}) in the upper right corner.
    ...
    If you already have a fork of the **development** branch of the [mendix/docs](https://github.com/mendix/docs) repository, the page opens in a web editor.    
    If you do not have a fork yet, you are prompted to fork the repository to propose changes. Click **Fork this repository**.

    {{< figure src="/attachments/community-tools/contribute-to-mendix-docs/fork-this-repository.png">}}

    The page opens in a web editor.
...
5. Enter a commit message, add more description if needed, then click **Propose changes**.
...
6. Make sure the pull request uses our **development** branch as the base.
    ...
    {{< figure src="/attachments/community-tools/contribute-to-mendix-docs/base-development.png">}}
     ...
     For more information, refer to [Creating a Pull Request](https://help.github.com/articles/creating-a-pull-request/) and [GitHub Standard Fork and Pull Request Workflow](https://gist.github.com/Chaser324/ce0505fbed06b947d962).
...
The pull request is created. The Mendix Documentation team will review the pull request as soon as possible. After the review is completed, and if there are no questions, the pull request is merged, and you will be able to see the changes on the Mendix Docs website!
...
2. Clone the forked repository using your preferred Git client, such as Visual Studio Code, GitHub Desktop, or Sourcetree:
    ...
    * Clone using Sourcetree:
     ...
     The repository is cloned into a folder called **docs** in the location you selected. You can rename it afterwards if you want to.

     {{% alert color="warning" %}}If you are running on Windows, avoid using a long directory path for the local directory. Some files can reach Git’s file length limits. A base path of up to 50 characters works, but 64 characters is currently too long.{{% /alert %}}
...
5. To run a local version of the site to preview your changes, in the root directory of the local repository, run the following command in a terminal: `npm run build`. 
...
9. Navigate in the left sidebar menu to the page that you just made changes to, and you can preview your changes locally.     
    As long as the current local build is still running, every new saved change is detected, and the site is rebuilt. Wait until the web server is available again, and you will be able to preview the changes you just saved.     
    If you do not see the changes you just saved, clear your browser cache, close and reopen the browser, and go to the local site again. Alternatively, in many browsers, such as Google Chrome, Firefox, and Microsoft Edge, you can perform a hard refresh on the current page by pressing <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>R</kbd> on a Windows PC, or <kbd>Command</kbd> + <kbd>Shift</kbd> + <kbd>R</kbd> on a Mac. This hard refresh ignores cached content.    
    If the above solutions do not work, stop the current running build in the terminal by pressing <kbd>Ctrl</kbd> + <kbd>C</kbd> and run the command `npm run build` again.
...
You are directed to our GitHub repo's [Issues](https://github.com/mendix/docs/issues) page. Please include the following details in your feedback:
...
* Make sure the Markdown file name reflects the title of the page. For example, if the page is titled "Creating Your App," the file name should be *creating-your-app.md*.
* Name the Markdown and image files in lower-case with a hyphen (`-`) between each word. Do not use spaces or any other symbols, such as `&`, in the file name, as these will not be accepted.
* When creating screenshots:
    ...
    * Make sure the image file has a name that makes sense so it is easy to find.
    * Save the image as a *.png* (preferred), *.jpg*, or *.gif* file.
    * Save the image file in a subfolder in the [attachments](https://github.com/mendix/docs/tree/development/static/attachments) folder for the location of the document. This subfolder should have the same name as the document file name you are working on.

### Submitting Your Work

When you are ready to submit your work, follow these guidelines:

* Commit new Markdown files via your preferred Git client or via the **Create new file** button in the [Mendix GitHub repo](https://github.com/mendix/docs), where you can copy and paste your text.
...
Finally, create a pull request that specifies the Mendix **development** branch as the base. For more information, refer to [Creating a Pull Request](https://help.github.com/articles/creating-a-pull-request/).

{{% alert color="info" %}}
You may receive a response to your pull request asking for verification of specific issues. We do this to make sure your documentation submission fits well with what we have.
{{% /alert %}}
