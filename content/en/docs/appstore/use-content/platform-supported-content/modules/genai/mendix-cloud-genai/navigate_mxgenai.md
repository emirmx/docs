...
The Token consumption page shows detailed graphs of the token consumption used for the GenAI resource. Use this overview to see the current usage, insights on the usage per day and compare current month with previous months. Understanding the token usage helps with choosing the correct plan.

### What are Tokens

Tokens is a measurement used by the GenAI model. A good rule of thumb is that 100 tokens are around 75 English words, however every model is different because of different ways text is being tokenized. Embedding resources only have input tokens, the tokens that are send to the model. Text generation resources contain both input and output tokens (text send to the model and generated by the model).
...
The token consumption graphs can be used to compare the current GenAI plan with the consumption of the resource. The GenAI plan defines the input and output limits per month, and these limits are shown in the MTD and 6-month views. Don't worry, we won’t shut down the resource when the limits are reached. Plans are defined per month, and unused tokens don’t transfer to the next month.
