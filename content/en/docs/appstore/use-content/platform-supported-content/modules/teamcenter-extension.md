...
{{% alert color="info" %}} The Teamcenter Extension currently does not display compound attributes from Teamcenter. A compound attribute refers to a property that can be displayed on one object, although it is defined and resides on a different object.{{% /alert %}}
