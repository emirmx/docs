...
1. Create the following domain model:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581378.png" class="no-border" >}}
...
5. Name the microflow *IVK_SetOrderToComplete*:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/8946808.png" class="no-border" >}}

6. Save the new menu item by clicking **OK**. You should see an empty microflow like this:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/8946316.png" class="no-border" >}}

7. Add an **Action** button to the toolbar of the orders overview:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/8946696.png" class="no-border" >}}
...
9. Set **On click** to **Call a microflow**, and then select the **IVK_SetOrderToComplete** microflow you just created:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/8946804.png" class="no-border" >}}

10. Add the following customer data to your app:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581374.png" class="no-border" >}}

11. Add the following order data to your app:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581373.png" class="no-border" >}}
...
1. Open the app **Security**:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581561.png" class="no-border" >}}

    This will open the **App Security** properties editor.
2. Switch the **Security level** to **Prototype / demo**:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581559.png" class="no-border" >}}

3. Go to the **Administrator** tab and set the primary administrator **Password**:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581565.png" class="no-border" >}}
...
2. On the **Module Security Type 'Security'** properties editor, click **New**:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581556.png" class="no-border" >}}

3. In the **New Module Role** dialog box, enter *Administrator* for the module role **Name**:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581555.png" class="no-border" >}}

4. Create the **User** module role:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581554.png" class="no-border" >}}
...
2. On the **App Security** dialog box, go to the **User roles** tab and double-click the **Administrator** user role:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581553.png" class="no-border" >}}

3. In the **Module roles** section of the **User Role 'Administrator'** properties editor, click **Edit**:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581552.png" class="no-border" >}}

4. In the **Select Module Roles** pop-up window, select the **Administrator** module role for all the modules:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581550.png" class="no-border" >}}

5. Repeat steps 2–4 for the **User** user role:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581548.png" class="no-border" >}}
...
2. Open the **Page access** tab:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581547.png" class="no-border" >}}

3. Check the pages according to the example shown here:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581546.png" class="no-border" >}}

4. On the **Microflow access** tab, check the microflows according to the example shown here:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581544.png" class="no-border" >}}
...
2. On the **App Security** dialog box, switch the security level to **Production**:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581543.png" class="no-border" >}}
...
2. On the **Module Security Type 'Security'** dialog box, open the **Entity Access** tab:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581541.png" class="no-border" >}}
...
1. Select all the entities and click **OK**:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581539.png" class="no-border" >}}
...
2. Set up the rule configuration:
    ...
    * **Member read and write rights**: **Read, Write**
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581572.png" class="no-border" >}}
...
1. On the **Module Security Type 'Security"'** dialog box, click **New** to create a new access rule for the User module role:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581538.png" class="no-border" >}}

2. Select the **Customer** entity and click **OK**:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581536.png" class="no-border" >}}
...
3. Set up the rule configuration:
    ...
    * **Default rights for new members**: **Read**
    ...
    {{% alert color="warning" %}}Be careful when setting default rights. If a new attribute or association is added, it will automatically have the designated access, which may not be what you want. See the [Access Rights](/refguide10/access-rules/#access-rights) section of *Access Rules* for more details.{{% /alert %}}

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581535.png" class="no-border" >}}
...
4. Adjust the rule for the **Order** and set up the rule configuration:
    ...
    * **Access rights** for each member: **Read, Write**
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581534.png" class="no-border" >}} 

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581533.png" class="no-border" >}}
...
An XPath constraint can be used to constrain the set of objects to which the access rule applies. If the XPath constraint is empty, the rule applies to all the objects of the entity. For more information, see [XPath Constraints](/refguide10/xpath-constraints/) in the *Studio Pro Guide*.

### Adding an Account with the User User Role

1. Click the **Accounts** in the **Administration** menu:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581532.png" class="no-border" >}}

2. On the **Accounts** screen, click **New user**:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581530.png" class="no-border" >}}

3. Add an account with a user role named **User**:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581528.png" class="no-border" >}}
...
1. Double-click the **Order** entity:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581527.png" class="no-border" >}}

2. On the **Properties of Entity 'MyFirstModule.Order'** editor, open the **Access rules** tab:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581525.png" class="no-border" >}}
...
3. Double-click the **User** module role to open the **Edit Access Rule of Entity 'MyFirstModule.Order'** editor and select the **XPath constraint** tab:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581523.png" class="no-border" >}}

4. To constrain the access of the financial administrator to only the open orders, add the following XPath statement:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581522.png" class="no-border" >}}

5. Click **OK**. The properties editor of your Order entity should look like this:

    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581521.png" class="no-border" >}}
...
7. If you sign in with the Test User account, you will see that only orders with the **Order status** of **Open** are shown in the overview:
    ...
    {{< figure src="/attachments/howto10/security/create-a-secure-app/18581520.png" class="no-border" >}}
