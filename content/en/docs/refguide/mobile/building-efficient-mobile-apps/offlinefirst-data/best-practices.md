...
Microflows can be called from offline apps by using [Call microflow](/refguide/call-microflow/) activities in your nanoflows to perform logic on the server. However, it works a bit different from when used in online profiles, these differences are explained below:
