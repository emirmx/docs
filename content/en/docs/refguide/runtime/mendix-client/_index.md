...
When an object is retrieved in the microflow using a Java Action or a microflow that was called dynamically in a Java Action, it is not part of the analyzed state and changes made on the client are not applied. This mainly occurs when updating objects in the client associated with the current user or session object that are subsequently retrieved from the database in a Java Action. Make sure to commit changes to those kinds of objects to the database before calling the microflow to prevent issues arising from this.
