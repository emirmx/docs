...
The **Reverse Merge Changes** option allows locally [rolling back changes](/refguide/using-version-control-in-studio-pro/#revert-changes) that were committed to the version control repository. These local changes can then be committed as a new revision.

## Revert All Changes {#revert-all-changes}

The **Revert All Changes** option allows [rolling back](/refguide/using-version-control-in-studio-pro/#revert-changes) all local changes, both in the project and in files on disk, that have been introduced since the last commit.
