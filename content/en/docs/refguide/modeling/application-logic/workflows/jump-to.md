...
For interrupting boundary event paths, in Studio Pro 10.21.0 and above, it is possible to jump within, into, or out of an interrupting boundary event path. If the interrupting boundary event is in a parallel split path, the boundary path must end with a **Jump** activity and then can jump back to the parent path (which is the parallel split path).
